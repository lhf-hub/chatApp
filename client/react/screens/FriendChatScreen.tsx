...
import { View, StyleSheet, TouchableOpacity, VirtualizedList, Alert } from "react-native";
...
import { createChat, getChatBy2Id, getDBConnection, getFriendMessages, insertMessage, readMessage } from "../SQLite/db-service";
...
interface FriendChatScreenProps {
    ...
    route: RouteProp<{ FriendChatScreen: { userData: UserWithRelation } }, 'FriendChatScreen'>
}
const FriendChatScreen = (props: FriendChatScreenProps) => {
        ...
        ImagePicker.launchImageLibrary(options, (response) => {
            if (response.didCancel) {
                ...
                sendMessage({
                    ...
                    receiverId: props.route.params.userData.id,
                    groupId: null,
    ...
    useEffect(() => {
        context.events.set('receiveMessage', onReceiveMessage)
        getMessages()
    }, [])
    ...
    const getMessages = async () => {
        try {
            const chat = await getChatBy2Id(await getDBConnection(), context.user.id, props.route.params.userData.id)
            if (!chat) {
                await createChat(await getDBConnection(), {
                    user_id: context.user.id,
                    another_id: props.route.params.userData.id,
                    type: 'friend',
                    last_message_id: null,
                    updated_at: Date.now().toLocaleString()
                })
            } else {
                const res = await getFriendMessages(await getDBConnection(), context.user.id, props.route.params.userData.id)
                const ids: string[] = []
                res.forEach(i => {
                    if (i.status == 'unread') {
                        ids.push(i.id)
    ...
    const sendMessage = async (message: any) => {
        ...
        context.socket.emit('sendMessage', JSON.stringify(message));
        Message.status = 'read'
        getDBConnection()
            .then(db => {
            ...
            <Appbar.Header style={styles.header}>
                ...
                {/* <Appbar.Content title={
                    <TextInput
                        ...
                        style={styles.input}
                        label={searchEnable ? 'Search' : ''}
                    />
                } /> */}
                <Appbar.Action color={context.theme.current.surface.On} icon="magnify" onPress={() => { setSearchEnable(!searchEnable); setSearchText('') }} />
            </Appbar.Header>
            <View style={{ flex: 1 }}>
